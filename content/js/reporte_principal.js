...
function reporte_doc() {
  ...
  $.ajax({
    ...
    for (var i = 0; i < 12; i++) {
      var cantidad = datosDesplazados.todos[i]?.cantidad || 0;
      var meta = datosDesplazados.todos[i]?.meta || 0;
      var porcentaje = meta > 0 ? (cantidad / meta * 100).toFixed(2) : 0;
    
      tablaHTML += `
        <tr>
          <td>${meses[i]}</td>
          <td>${datosDesplazados.entrada[i] || 0}</td>
    ...
    tablaHTML += `
      ...
      // Inserta la tabla completa en el contenedor deseado
      ...
      if (!$.fn.DataTable.isDataTable('#tablaDocumentos')) {
        var table = $('#tablaDocumentos').DataTable({      
          ...
          dom: "<'row'<'col-12 d-flex justify-content-between align-items-center'<B><'my-2'f>>>" +
          "<'row'<'col-12'tr>>" +
          "<'row'<'col-12 d-flex justify-content-between align-items-center'<il><p>>>",
          ...
          buttons:[ 
        {
          ...
          title: function() {
            ...
            return searchString.length? "Search: " + searchString : "Reporte de Documentos"
          },
        ...
        },
        {
          ...
          title: function() {
            var total = table.column(4).data().reduce(function (a, b) {
                return parseFloat(a) + parseFloat(b);
            }, 0);
            var searchString = table.search();        
            return searchString.length? "Search: " + searchString : "Reporte de Documentos (Total de Documentos: " + total + ")"
          },
        ...
        },
        {
          ...
          title: function() {
            var searchString = table.search();        
            return searchString.length? "Search: ": "Reporte de documentos"
          },
